...
/**
 * Get all non-deleted files, optionally filtered by category
 */
export async function getFiles(category?: string) {
  ...
  let query: any = db.select().from(files).where(isNull(files.deletedAt));
  
  if (category) {
    query = db.select().from(files).where(isNull(files.deletedAt));
  }

  return query.orderBy(files.uploadedAt);
}
...
/**
 * Soft delete a file (mark as deleted without removing from DB)
 */
